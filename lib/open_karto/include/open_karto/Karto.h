...
public:
    ...
    Matrix3 Inverse() const
    {
        ...
        // std::cout << kInverse << std::endl;

        

        // start_time_ = std::chrono::steady_clock::now();

        kt_bool haveInverse = InverseFast(kInverse, 1e-20);

        // end_time_ = std::chrono::steady_clock::now();
        // time_used_ = std::chrono::duration_cast<std::chrono::duration<double>>(end_time_ - start_time_);
        // std::cout << "定义求逆用时: " << time_used_.count() << " 秒。" << std::endl;

        try
        {
            if (haveInverse == false)
            {
                // std::cout << "矩阵不可逆，使用伪逆" << std::endl;
                // assert(false);
                Eigen::Matrix3d temp_matrix;
                temp_matrix << kInverse.m_Matrix[0][0], kInverse.m_Matrix[0][1], kInverse.m_Matrix[0][2],
                               kInverse.m_Matrix[1][0], kInverse.m_Matrix[1][1], kInverse.m_Matrix[1][2],
                               kInverse.m_Matrix[2][0], kInverse.m_Matrix[2][1], kInverse.m_Matrix[2][2];

                // std::cout << std::endl << temp_matrix << std::endl;
                ...
                // std::cout << std::endl << temp_matrix.inverse() << std::endl;

                // end_time_ = std::chrono::steady_clock::now();
                // time_used_ = std::chrono::duration_cast<std::chrono::duration<double>>(end_time_ - start_time_);
                // std::cout << "Eigen求逆用时: " << time_used_.count() << " 秒。" << std::endl;


                // start_time_ = std::chrono::steady_clock::now();

                // 求矩阵伪逆
                Eigen::JacobiSVD<Eigen::MatrixXd> svd(temp_matrix, Eigen::ComputeThinU | Eigen::ComputeThinV);
                ...
                Eigen::Matrix3d pinv_matrix = V * S * U.transpose();

                // if(abs(pinv_matrix(0,0)) >= 1e+20 || abs(pinv_matrix(0,1)) >= 1e+20 || abs(pinv_matrix(1,0)) >= 1e+20 || abs(pinv_matrix(1,1)) >= 1e+20 || abs(pinv_matrix(2,2)) >= 1e+20)
                // {
                //     pinv_matrix /= 1e+20;
                // }

                kInverse.m_Matrix[0][0] = pinv_matrix(0,0);
                kInverse.m_Matrix[0][1] = pinv_matrix(0,1);
                kInverse.m_Matrix[0][2] = pinv_matrix(0,2);
                kInverse.m_Matrix[1][0] = pinv_matrix(1,0);
                kInverse.m_Matrix[1][1] = pinv_matrix(1,1);
                kInverse.m_Matrix[1][2] = pinv_matrix(1,2);
                kInverse.m_Matrix[2][0] = pinv_matrix(2,0);
                kInverse.m_Matrix[2][1] = pinv_matrix(2,1);
                kInverse.m_Matrix[2][2] = pinv_matrix(2,2);


                std::cout << "kInverse: " << kInverse << std::endl;

                // std::cout << std::endl << V * S * U.transpose() << std::endl;

                // end_time_ = std::chrono::steady_clock::now();
                // time_used_ = std::chrono::duration_cast<std::chrono::duration<double>>(end_time_ - start_time_);
                // std::cout << "Eigen求伪逆用时: " << time_used_.count() << " 秒。" << std::endl;

            }
        }
        catch(Exception e)
        {
            std::cout << "e: " << e.GetErrorMessage() << std::endl;
        }
