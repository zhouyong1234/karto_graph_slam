...
#include "karto_graph_slam/karto_slam.h"
...
void SlamKarto::publishGraphVisualization()
{
    ...
    // std::cout << "use_back_end: " << use_back_end_ << " solver_type: " << solver_type_ << std::endl;

    if (use_back_end_ && solver_type_ == "ceres_solver")
    {
      std::vector<Constraint2d> constraints;
      std::map<int, Pose2d> poses;

      static_cast<CeresSolver*>(solver_)->getGraph(constraints, poses);

      if(constraints.empty() || poses.empty())
      {
          // std::cerr << "No constraints, no problem to optimize." << std::endl;
          return;
      }
    ...
    else if(use_back_end_ && solver_type_ == "gtsam_solver")
    {
      std::vector<Eigen::Vector2d> graph_nodes;
      std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d> > graph_edges;

      static_cast<GTSAMSolver*>(solver_)->getGraph(graph_nodes, graph_edges);
    ...
    else if(use_back_end_ && solver_type_ == "g2o_solver")
    {
      std::vector<Eigen::Vector2d> graph_nodes;
      std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d> > graph_edges;

      static_cast<G2OSolver*>(solver_)->getGraph(graph_nodes, graph_edges);
    ...
    else if(use_back_end_ && solver_type_ == "spa_solver")
    {

      // std::cout << "pose graph spa" << std::endl;
